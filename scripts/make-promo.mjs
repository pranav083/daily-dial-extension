...
try {
  ...
  await sleep(2800);   // let the last hint toast clear before this scene
  await caption("Then see whether it went where you meant.");
  await hold(2.6);

  await caption("Or type it, if that is faster.");
  await page.evaluate(`(() => { const i = document.getElementById("typed-entry-input"); i.focus(); i.value = ""; })()`);
  for (const ch of "19-20 admin") {
    await page.evaluate(`(() => { const i = document.getElementById("typed-entry-input");
  ...
  await sleep(400);
  await hold(1.6);
  // Let the "Added" toast finish before the next scene, rather than carrying
  // it into the history view where it has nothing to do with anything.
  await sleep(2600);
  await hold(0.4);
